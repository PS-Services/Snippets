...
class ResultItem
{
  ...
  [string]ToString() { return $this.Line; }
}
...
class PackageManager
{
    ...
    [bool]$Sudo = $false)
  {
    ...
    return $this.ParseResults(
      $executeResults, $Command, $Install, $AllRepos, $Raw, $Describe, $Verbose);
  }
    ...
    [bool]$Exact = $false)
  {
    ...
    $results = $this.Execute($itemCommand, $params, $Install, $AllRepos, $Raw, $Describe, $Verbose, $Sudo)
...
class NpmManager : PackageManager
{
  ...
  [Object[]]AddParameters([Object[]]$params)
  {
    if($params[0] -imatch 'search|find'){
        $params += '--json';
    }
